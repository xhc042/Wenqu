...
4. **importance** - 重要性：1-5的整数
   - 必须有区分度：整本书里 importance=5 的章节不超过2个，importance=4 的不超过3个，其余给2或3
   - 5=全书最核心，是后续章节的基础；1=扩展补充内容；避免所有章节都打3
5. **difficulty** - 难度：简单/中等/较难
...
任务4：识别全书最核心的3-5个正文章节
   - **参考快照中的重要性评分**：优先选 importance 最高的正文章节
...
任务4：识别全书最核心的4-6个正文章节（占全书价值80%）
   - **参考快照中的重要性评分**：优先选 importance 最高的正文章节
   - **绝对不能包含序言/前言/自序/附录等元数据章节**
任务5：用50字以内描述全书的核心逻辑关系

...
## 测试覆盖率目标 (更新于 2026-06-30)

| 模块 | 当前 | 目标 | 状态 |
|------|------|------|------|
| **测试代码 (P0/P1)** | **96-100%** | 90%+ | ✅ 达标 |
| **chunker.py** | 37% | 90%+ | ⚠️ 待提升 |
| **state_machine.py** | 40% | 95%+ | ⚠️ 待提升 |
| **database.py** | 33% | 75%+ | ⚠️ 待提升 |
| **llm_client.py** | 36% | 70%+ | ⚠️ 待提升 |
| **config.py** | 94% | 80%+ | ✅ 超标 |
| **app.py** | 23% | 50%+ (集成) | ⚠️ 待提升 |
| **routes/*.py** | 13-27% | 50%+ | ⚠️ 待提升 |

> 💡 **说明**: 测试代码覆盖率指 `tests/` 目录下的测试文件自身覆盖率；生产代码覆盖率指所有非测试文件的覆盖率。当前项目测试代码覆盖率达 96-100%，但生产代码整体覆盖率仅 46%，需重点补充 HTTP 路由、WebSocket、数据库 CRUD 等集成测试。
